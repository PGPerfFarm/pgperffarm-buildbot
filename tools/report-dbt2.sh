...
tail -n +2 "${CSVEXPORT}" | while IFS= read -r LINE; do
	...
	NOTPM="$(curl --silent "${BUILDBOTURL}/api/v2/logs/${LOGSID}/raw_inline" \
			| sed -n 's/.*Throughput: \([0-9]\+\(\.[0-9]\+\)\?\).*/\1/p')"

	echo "${LINE} ${CTIME} ${NOTPM}" >> "${CSVREPORT}"
done
...
for PLANT in ${PLANTS}; do
	...
	gnuplot <<- __PLOT__
		...
		set grid
		set title "DBT-2 Results ${PLANT}" noenhanced
		set output 'dbt2-${PLANT}.png'
